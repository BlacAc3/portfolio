    ...
    <section id="about" className="py-20 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        <BentoGrid>
            ...
            <BentoCard colSpan={2} rowSpan={2} theme="dark" className="h-full">
              <div className="h-full flex flex-col pointer-events-none">
                <div className="flex justify-between items-center mb-8">
                  <h4 className="text-[10px] uppercase tracking-[0.4em] font-black text-chocolate-accent">Professional Journey</h4>
                  <span className="text-[10px] font-black text-white/20 group-hover:text-chocolate-accent transition-colors uppercase tracking-widest font-tech">Explore</span>
                </div>
                <div className="space-y-8">
                  {experiences.slice(0, 3).map((exp, i) => (
                    <div key={i} className="border-l border-white/10 pl-6 pb-2">
                      <div className="flex justify-between items-start mb-2">
                        <p className="font-black text-xl text-white">{exp.company}</p>
                        <span className="text-[10px] uppercase font-bold text-white/30">{exp.period}</span>
                      </div>
                      <p className="text-chocolate-accent text-sm font-bold">{exp.role}</p>
                    </div>
                  ))}
                </div>
              </div>
          ...
          >
            <BentoCard colSpan={1} rowSpan={1} theme="accent" className="h-full">
                <div className="h-full flex flex-col justify-between pointer-events-none">
                    <div className="flex justify-between items-center">
                        <h4 className="text-[10px] uppercase tracking-[0.3em] font-black opacity-60 font-tech">Toolkit</h4>
                        <span className="text-[8px] font-black opacity-40 group-hover:opacity-100 transition-opacity uppercase tracking-widest font-tech">Details</span>
                    </div>
                    <div className="flex flex-wrap gap-2">
                        {skills.slice(0, 4).map((skill) => (
                        <span key={skill.name} className="px-2 py-1 bg-black/10 rounded text-[10px] font-black font-tech">
                            {skill.name}
                        </span>
                        ))}
                    </div>
          ...
          <BentoCard colSpan={1} rowSpan={2} className="p-0 border-0 overflow-hidden">
            <img src={me} alt="Aaron" className="w-full h-full object-cover grayscale hover:grayscale-0 transition-all duration-1000 group-hover:scale-105" />
          </BentoCard>
          ...
          <BentoCard colSpan={1} rowSpan={1} theme="glass">
            <div className="h-full flex flex-col justify-between">
              <h3 className="text-xl font-black leading-tight">
                Bridging <span className="text-chocolate-accent">logic</span> & aesthetics.
              </h3>
              <div className="flex gap-3">
                <a href="https://github.com/blacac3" target="_blank" className="p-2 bg-white/5 rounded-xl hover:bg-white hover:text-black transition-all">
                  <FaGithub size={16} />
                </a>
                <a href="https://linkedin.com/in/aaron-ezeala" target="_blank" className="p-2 bg-white/5 rounded-xl hover:bg-white hover:text-black transition-all">
                  <FaLinkedin size={16} />
                </a>
              </div>
            </div>
          ...
          <BentoCard colSpan={2} rowSpan={1} theme="glass">
            <div className="grid grid-cols-2 gap-8 h-full items-center">
              <div className="text-center border-r border-white/5">
                <p className="text-5xl font-black text-chocolate-accent tracking-tighter font-tech">20+</p>
                <p className="text-[10px] uppercase tracking-widest font-bold opacity-30 mt-2 font-tech">Projects Delivered</p>
              </div>
              <div className="text-center">
                <p className="text-5xl font-black text-white tracking-tighter font-tech">3+</p>
                <p className="text-[10px] uppercase tracking-widest font-bold opacity-30 mt-2 font-tech">Years Experience</p>
              </div>
            </div>
          </BentoCard>
        ...
        <AnimatePresence>
          {expandedType && (
            <>
              ...
              />
              <motion.div
                layoutId={expandedType === 'journey' ? "journey-card" : "toolkit-card"}
                className="fixed top-12 md:top-20 inset-x-0 bottom-0 bg-chocolate-dark/40 backdrop-blur-2xl z-[200] overflow-hidden flex flex-col rounded-t-[40px] shadow-[0_-20px_80px_-20px_rgba(0,0,0,0.9)] border-t border-x border-white/10"
              >
                <div className="p-8 md:p-12 md:px-20 flex justify-between items-center border-b border-white/5 bg-white/2">
                   <div>
                    <h4 className="text-xs uppercase tracking-[0.6em] font-black text-chocolate-accent mb-2 font-tech">
                        {expandedType === 'journey' ? 'Professional Journey' : 'Technical Proficiency'}
                    </h4>
                    <p className="text-3xl md:text-5xl font-black tracking-tighter text-white uppercase">
                        {expandedType === 'journey' ? 'Full Timeline' : 'The Toolkit'}
                    </p>
                   </div>
                   <button 
                     onClick={() => setExpandedType(null)}
                        ...
                        {experiences.map((exp, i) => (
                          ...
                          >
                            ...
                            <div className="flex flex-col md:flex-row justify-between items-start mb-6 gap-4">
                              <div>
                                <p className="font-black text-3xl md:text-4xl text-white mb-2 tracking-tight">{exp.company}</p>
                                <p className="text-chocolate-accent text-lg font-bold font-tech uppercase tracking-widest">{exp.role}</p>
                              </div>
                              <span className="px-4 py-2 bg-white/5 rounded-full text-xs font-black text-white/40 font-tech whitespace-nowrap">{exp.period}</span>
                            </div>
                            <ul className="space-y-4">
                              {exp.details.map((detail, j) => (
                                <li key={j} className="text-lg text-white/60 leading-relaxed font-light flex gap-4">
                                  <span className="text-chocolate-accent font-black mt-1">/</span> {detail}
                                </li>
                              ))}
                            </ul>
                          </motion.div>
                         ...
                         {skills.map((skill, i) => (
                           ...
                           >
                              <div className="flex justify-between items-end mb-4">
                                <div>
                                  <p className="text-[10px] uppercase tracking-widest font-black text-white/20 mb-1 font-tech">{skill.category}</p>
                                  <p className="text-2xl font-black text-white tracking-tight">{skill.name}</p>
                                </div>
                                <p className="text-2xl font-black text-chocolate-accent font-tech">{skill.level}%</p>
                              </div>
                ...
                <div className="p-8 border-t border-white/5 bg-white/2 text-center">
                   <p className="text-[10px] uppercase tracking-[0.4em] font-black text-white/20">End of Overview — Focused on Impact</p>
                </div>
              </motion.div>
            </>
          )}
